...
RNG_PSEUDO = 'CURAND_RNG_PSEUDO_DEFAULT'
RNG_QUASI  = 'CURAND_RNG_QUASI_SOBOL32'
...
class CURAND_Base(GpuOp):
    ...
    """
    def __init__(self, output_type, seed, destructive, rng_type):
        """
        output_type: a theano type (e.g. tensor.fvector)
        seed: integer (used as seed for RNG_PSEUDO, as dimension for RNG_QUASI)
        destructive: True or False (on the generator)
        rng_type: rng_curand.RNG_PSEUDO or rng_curand.RNG_QUASI
        """
    ...
    def as_destructive(self):
        ...
        return self.__class__(self.output_type, self.seed, destructive=True, rng_type=self.rng_type)
    ...
    @classmethod
    def new_auto_update(cls, generator, ndim, dtype, size, seed, rng_type):
        """
    ...
    def c_code(self, node, nodename, inp, out, sub):
        ...
        generator_type = self.rng_type
        if self.rng_type == 'CURAND_RNG_PSEUDO_DEFAULT':
            generator_setup = "curandSetPseudoRandomGeneratorSeed(*gen,%s)" % seed
            generator_setup_msg = "Failed to set curand generator seed"
        elif self.rng_type == 'CURAND_RNG_QUASI_SOBOL32':
            generator_setup = 'curandSetQuasiRandomGeneratorDimensions(*gen,%d)' % seed
            generator_setup_msg = 'Failed to set QRNG dimension'
        else:
            raise ValueError("Unknown rng_type %s" % (self.rng_type,))
        ...
        if (!PyCObject_Check(%(i_generator)s))
        {
            ...
            if (CURAND_STATUS_SUCCESS !=
                    curandCreateGenerator(gen, %(generator_type)s)) {
            ...
            if (CURAND_STATUS_SUCCESS !=
                    %(generator_setup)s)
            {
                PyErr_Format(PyExc_RuntimeError, "%(generator_setup_msg)s");
                %(fail)s;
            }
...
class CURAND_RandomStreams(object):
    ...
    def __init__(self, seed, rng_type=RNG_PSEUDO):
        """ seed: int
        """
